...
public struct WebpDecoderConfig: InternalRawRepresentable {
    ...
    public init() {
        var originConfig = WebPDecoderConfig()
        if (WebPInitDecoderConfig(&originConfig) == 0) {
            fatalError("can't init decoder config")
        }
        self.init(rawValue: originConfig)!
    }

    internal init?(rawValue: WebPDecoderConfig) {
    ...
    internal var rawValue: WebPDecoderConfig {
        return WebPDecoderConfig(input: (input?.rawValue)!, output: output.rawValue, options: options.rawValue)
    }
}
...
public struct WebpBitstreamFeatures: InternalRawRepresentable {
    ...
    internal var rawValue: WebPBitstreamFeatures {
    ...
    internal init?(rawValue: WebPBitstreamFeatures) {
...
public struct WebpDecBuffer: InternalRawRepresentable {
    ...
    internal var rawValue: WebPDecBuffer {
        let originU: WebPDecBuffer.__Unnamed_union_u
        switch u {
        case .RGBA(let buffer):
            originU = WebPDecBuffer.__Unnamed_union_u(RGBA: buffer)
        case .YUVA(let buffer):
            originU = WebPDecBuffer.__Unnamed_union_u(YUVA: buffer)
        }
    ...
    internal init?(rawValue: WebPDecBuffer) {
...
public struct WebpDecoderOptions: InternalRawRepresentable {
    ...
    internal var rawValue: WebPDecoderOptions {
    ...
    internal init?(rawValue: WebPDecoderOptions) {
